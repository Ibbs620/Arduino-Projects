/*
 * Program for a 2 player pong game for the Arduino Uno
 * 
 * This code assumes you are using the MAX7219 LED Dot Matrix. If you have the standard one, you may need to reprogram.
 * 
 * Pins:
 * 
 * 2 - Player 1 Down Button
 * 3 - Player 1 Up Button
 * 5 - Player 2 Up Button
 * 6 - Player 2 Down Button
 * 9 - Piezo
 * 10 - CS (LED Matrix)
 * 11 - CLK (LED Matrix)
 * 12 - DIN (LED Matrix)
 * 
 * Other notes:
 * - When connecting buttons, connect two adjacent pins to ground and digital pins. Do not use the other 2 pins
 * - The dot matrix is placed with the pins facing upwards.
 * - You may need to hold the up and down buttons to move the paddle rather than pressing it quickly.
...
void loop() {
  noTone(9);
  int s;
  if(!digitalRead(3) && p2 < 224){
    p2 *= 2;
    pad2++;
  } else if(!digitalRead(2) && p2 > 7){
    p2 /= 2;
    pad2--;
  }
  if(!digitalRead(5) && p1 < 224){
    p1 *= 2;
    pad1++;
  } else if(!digitalRead(6) && p1 > 7){
    p1 /= 2;
    pad1--;
  }
